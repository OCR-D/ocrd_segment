...
Usage: import the module, or run from the command line as such:
    ...
    maskrcnn-address --model=last --split 0 predict --dataset=/path/to/coco.json --dataset-pred=/path/to/coco-pred.json

    # Merge and sort multiple COCO datasets (for comparison)
    maskrcnn-address merge --dataset=/path/to/coco*.json --dataset-merged=/path/to/coco-merged.json --sort --rle
    maskrcnn-address merge --dataset=/path/to/coco-pred*.json --dataset-merged=/path/to/coco-pred-merged.json --sort --rle
...
def main():
    ...
    predict_parser.add_argument('--dataset', required=True, metavar="PATH/TO/COCO.json",
                                help='File path of the address annotations ground truth dataset to be read (randomly split into skip and prediction)')
    ...
    merge_parser.add_argument('--dataset', required=True, metavar="PATH/TO/COCO.json", nargs='+',
                              help='File path(s) of the address annotations dataset(s) to be read')
